...
object TableAggFunctionTest {
  ...
  // 自定义一个表聚合函数，实现Top2功能，输出（temp，rank）   class Top2Temp() extends TableAggregateFunction[(Double, Int), Top2TempAcc]{
  // 初始化状态
  // 每来一个数据后，聚合计算的操作   def accumulate( acc: Top2TempAcc, temp: Double ): Unit
  // 将当前温度值，跟状态中的最高温和第二高温比较，如果大的话就替换
  // 如果比最高温还高，就排第一，其它温度依次后移
  // 实现一个输出数据的方法，写入结果表中   def emitValue( acc: Top2TempAcc, out: Collector[(Double, Int)] ): Unit
